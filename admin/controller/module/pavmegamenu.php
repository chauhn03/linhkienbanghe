...
class ControllerModulePavmegamenu extends Controller {
	...
	public function ajxmenuinfo(){
		...
		if( isset($this->request->post['params']) ) {
		 ...
		 //	echo '<pre>'.print_r( $a, 1 ); die;
			$this->model_setting_setting->editSetting( 'pavmegamenu_params', array('params'=> $params) );
	...
	public function liveedit(){
		...
		$this->document->addStyle(  HTTPS_CATALOG.'catalog/view/theme/default/stylesheet/pavmegamenu/css/bootstrap.css');
		$this->document->addScript( HTTPS_CATALOG.'catalog/view/theme/default/stylesheet/pavmegamenu/js/bootstrap.js');
		...
		$this->document->addStyle( HTTPS_CATALOG.'catalog/view/theme/default/stylesheet/pavmegamenu/style.css');
		...
		$this->data['liveedit_action'] = $this->url->link('module/pavmegamenu/livesave', 'root=1'.$store_param.'&token=' . $this->session->data['token'], 'SSL');
		$this->data['action_backlink'] = $this->url->link('module/pavmegamenu', 'root=1'.$store_param.'&token=' . $this->session->data['token'], 'SSL');
		$this->data['action_widget'] =  HTTPS_CATALOG.'index.php?route=module/pavmegamenu/renderwidget';
		$this->data['action_addwidget'] = $this->url->link('module/pavmegamenu/addwidget', 'token=' . $this->session->data['token'].$store_param, 'SSL'); 


		$this->data['ajxgenmenu'] 	   = $this->url->link('module/pavmegamenu/ajxgenmenu', 'root=1'.$store_param.'&token=' . $this->session->data['token'], 'SSL'); 
		$this->data['ajxmenuinfo'] 	   = $this->url->link('module/pavmegamenu/ajxmenuinfo', 'root=1'.$store_param.'&token=' . $this->session->data['token'], 'SSL'); 
	...
	public function ajxgenmenu( ){ 
		...
		if( isset($this->request->post['reset']) && $this->hasPermssion() ){
	 		$this->model_setting_setting->editSetting( 'pavmegamenu_params', array('params'=>'') ); 
	 	}
		...
		if (isset($this->request->get['store_id'])){
	 	...
	 	$this->data['treemenu'] = $this->model_menu_tree->getTree( 1, true, $params, $store_id );
	...
	public function addwidget(){
		...
		$this->data['action'] = $this->url->link('module/pavmegamenu/addwidget', 'token=' . $this->session->data['token'], 'SSL'); 
