...
class ControllerModulePavproductcarousel extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('pavproductcarousel', $this->request->post);		
					
			$this->session->data['success'] = $this->language->get('text_success');
			if( $action !='' ) { 			
				$this->redirect($this->url->link('module/pavproductcarousel', 'token=' . $this->session->data['token'], 'SSL'));		
			}else {
		...
		$this->data['tooltip_placement'] = array('top', 'bottom', 'left', 'right', 'auto');
		...
		if (isset($this->request->post['pavproductcarousel_module'])) {
			$this->data['modules'] = $this->request->post['pavproductcarousel_module'];
		} elseif ($this->config->get('pavproductcarousel_module')) { 
			$this->data['modules'] = $this->config->get('pavproductcarousel_module');
		}	
		...
		$this->data['tabs'] = $tabs;
		$this->template = 'module/pavproductcarousel.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function validate() {
		...
		if (isset($this->request->post['pavproductcarousel_module'])) {
			foreach ($this->request->post['pavproductcarousel_module'] as $key => $value) {
